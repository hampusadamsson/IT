...
%% @doc Prints the Integer N followed by Hello!~n
%% Then "N-1 Hello!~n" is printed,
%% down to "1 Hello!~n".
%% When N = 0, "ok" is printed.
%% === Example ===
%% <div class="example">```
%% 73> tutorial:hello(4).
%% 4 Hello!
...
simpsons(names) ->
    [C || {A,B,C} <- simpsons()];
simpsons(males) ->
    [C || {A,B,C} <- simpsons(), B == male];
simpsons(females) ->
    [C || {A,B,C} <- simpsons(), B == female];
simpsons(pets) ->
    [C || {A,B,C} <- simpsons(), A /= person].
