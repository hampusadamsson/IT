...
%% @doc Returns fifo with X inserted
%% X: add a -spec type declaration

%% To make it fast to push new values, add a new value to the head of
%% In.
-spec push(fifo(), _) -> fifo().

push({fifo, In, Out}, X) -> 
    {fifo, [X|In], Out}.

%% @doc returns fifo with one object popped
%% @throws 'empty fifo'
%% TODO: add a -spec type declaration

%% pop should return {Value, NewFifo}
-spec pop(fifo()) -> {_, fifo()}.
...
pop({fifo, In, []}) ->
    fifo:pop({fifo, [], lists:reverse(In)}).


%% @doc Returns true if fifo empty else false
