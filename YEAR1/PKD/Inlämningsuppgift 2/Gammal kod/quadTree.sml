...
POST: Quadträdet q med rektangeln r insatt på rätt plats i trädet.
EXAMPLE:
VARIANT: 
*)
...
fun query (EmptyQuadTree, x,y) = []
  | query (Qt(Extent as Rect(eLeft,eTop,eRight, eBottom),H,V,tL,tR,bL,bR),x,y) = 
    ...
    in
	if x < VA andalso y > HA then query'(H,x,y)@ query'(V,x,y)::query(tL,x,y)
	else if 

	    else query'(H,x,y)@ query'(V,x,y)::query(tR,x,y)
	else if

	    else query'(H,x,y)@ query'(V,x,y)::query(bL,x,y)
	else if
